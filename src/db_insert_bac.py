...
def load_data(data_file, schema_file):
    ...
    file = open(data_file, "r", encoding="utf-8", newline="")
    ...
    for row in tqdm(spamreader, total=num_rows, unit="row", desc="Processing rows"):
        if len(row) != len(schema):
            raise Exception(
                f"Error while parsing data file '{data_file}' at line {row}, expected {len(schema)} values, but found {len(row)}"
            )

        entry = {}

        for (name, type), value in zip(schema, row):
            if type == "num":
                try:
...
if __name__ == "__main__":
    ...
    if not args.preserve_existing_db_entries:
        cur.execute(f"SELECT COUNT(*) FROM bac WHERE an = {args.year}")
        cnt = cur.fetchone()[0]
